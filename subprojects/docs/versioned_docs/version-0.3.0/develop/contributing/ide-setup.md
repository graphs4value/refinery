...
1. See the [required tools](/develop/contributing#required-tools) for compiling Refinery on obtaining the required JDK version. You'll also need a version of IntelliJ IDEA that supports **Java 25** (version **2025.2** or later).
...
3. IntelliJ IDEA should build and index the project. If there are errors, it is likely that the `JAVA_HOME` was incorrectly set:
    * In _Project Structure > Project settings > Project > SDK_, a Java 25 compatible JDK should be selected.
    * In _Project Structure > Project settings > Project > Language level_, either _SDK default_ or _25_ should be selected.
...
5. Open _Window > Preferences_ and set the following preferences:
    ...
    * Add the JDK 25 to _Java > Installed JREs_.
    * Make sure JDK 25 is selected for _JavaSE-25_ at _Java > Installed JREs > Execution Environments_.
    * Set _Gradle > Java home_ to the `JAVA_HOME` directory (the directory which contains the `bin` directory) of JDK 25. Here, Buildship will show a yellow warning sign, which can be safely ignored.
    * Set _Java > Compiler > JDK Compliance > Compiler compliance level_ to _25_.
