...
public class XorTerm<T extends Xor<T>> extends BinaryTerm<T, T, T> {
	public XorTerm(Class<T> type, Term<T> left, Term<T> right) {
	...
	protected Term<T> constructWithSubTerms(Term<T> newLeft, Term<T> newRight) {
		return new XorTerm<>(getType(), newLeft, newRight);
	}
