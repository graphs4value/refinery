...
public class AbstractDomainLowerBoundTerm<A extends ComparableAbstractValue<A, C>, C extends Comparable<C>>
	...
	protected AbstractDomainLowerBoundTerm(ComparableAbstractDomain<A, C> abstractDomain, Term<A> body) {
		super(abstractDomain.abstractType(), abstractDomain, body);
	}

	@Override
	protected A doEvaluate(A bodyValue) {
		return bodyValue.abstractLowerBound();
	}

	@Override
	protected Term<A> constructWithBody(Term<A> newBody) {
		return new AbstractDomainLowerBoundTerm<>((ComparableAbstractDomain<A, C>) getAbstractDomain(), newBody);
	}

	@Override
	public String toString() {
		return "lowerBound(%s)".formatted(super.toString());
	}
}
