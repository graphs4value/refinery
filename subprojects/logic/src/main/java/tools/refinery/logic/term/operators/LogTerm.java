/*
 * SPDX-FileCopyrightText: 2025 The Refinery Authors <https://refinery.tools/>
 *
 * SPDX-License-Identifier: EPL-2.0
 */
package tools.refinery.logic.term.operators;

import tools.refinery.logic.term.Term;
import tools.refinery.logic.term.UnaryTerm;

public class LogTerm<T extends Log<T>> extends UnaryTerm<T, T> {
	public LogTerm(Class<T> type, Term<T> body) {
		super(type, type, body);
	}

	@Override
	protected T doEvaluate(T bodyValue) {
		return bodyValue.log();
	}

	@Override
	protected Term<T> constructWithBody(Term<T> newBody) {
		return new LogTerm<>(getType(), newBody);
	}

	@Override
	public String toString() {
		return "log(%s)".formatted(getBody());
	}
}
