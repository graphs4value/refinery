...
public class SubTerm<T extends Sub<T>> extends BinaryTerm<T, T, T> {
	public SubTerm(Class<T> type, Term<T> left, Term<T> right) {
	...
	protected Term<T> constructWithSubTerms(Term<T> newLeft, Term<T> newRight) {
		return new SubTerm<>(getType(), newLeft, newRight);
	}
