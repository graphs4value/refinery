...
public class MulTerm<T extends Mul<T>> extends BinaryTerm<T, T, T> {
	public MulTerm(Class<T> type, Term<T> left, Term<T> right) {
	...
	protected Term<T> constructWithSubTerms(Term<T> newLeft, Term<T> newRight) {
		return new MulTerm<>(getType(), newLeft, newRight);
	}
