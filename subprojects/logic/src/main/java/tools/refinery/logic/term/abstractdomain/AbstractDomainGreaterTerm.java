...
public class AbstractDomainGreaterTerm<A extends ComparableAbstractValue<A, C>, C extends Comparable<C>>
		extends AbstractDomainBinaryTerm<TruthValue, A, C> {
	public AbstractDomainGreaterTerm(ComparableAbstractDomain<A, C> abstractDomain, Term<A> left, Term<A> right) {
		super(TruthValue.class, abstractDomain, left, right);
	}

	@Override
	protected TruthValue doEvaluate(A leftValue, A rightValue) {
		return rightValue.checkLess(leftValue);
	}

	@Override
	protected Term<TruthValue> constructWithSubTerms(Term<A> newLeft, Term<A> newRight) {
		return new AbstractDomainGreaterTerm<>((ComparableAbstractDomain<A, C>) getAbstractDomain(), newLeft, newRight);
	}
