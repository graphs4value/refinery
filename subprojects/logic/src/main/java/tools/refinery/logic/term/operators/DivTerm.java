...
public class DivTerm<T extends Div<T>> extends BinaryTerm<T, T, T> {
	public DivTerm(Class<T> type, Term<T> left, Term<T> right) {
	...
	protected Term<T> constructWithSubTerms(Term<T> newLeft, Term<T> newRight) {
		return new DivTerm<>(getType(), newLeft, newRight);
	}
