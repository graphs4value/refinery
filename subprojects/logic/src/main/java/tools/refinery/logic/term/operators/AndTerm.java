...
public class AndTerm<T extends And<T>> extends BinaryTerm<T, T, T> {
	public AndTerm(Class<T> type, Term<T> left, Term<T> right) {
	...
	protected Term<T> constructWithSubTerms(Term<T> newLeft, Term<T> newRight) {
		return new AndTerm<>(getType(), newLeft, newRight);
	}
