...
public class AbstractDomainEqTerm<A extends AbstractValue<A, C>, C>
		extends AbstractDomainBinaryTerm<TruthValue, A, C> {

	public AbstractDomainEqTerm( AbstractDomain<A, C> abstractDomain, Term<A> left, Term<A> right) {
		super(TruthValue.class, abstractDomain, left, right);
	}

	@Override
	protected TruthValue doEvaluate(A leftValue, A rightValue) {
		return leftValue.checkEquals(rightValue);
	}

	@Override
	protected Term<TruthValue> constructWithSubTerms(Term<A> newLeft, Term<A> newRight) {
		return new AbstractDomainEqTerm<>(getAbstractDomain(), newLeft, newRight);
	}
