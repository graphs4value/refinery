...
class VariableDirectionTest {
	...
	private static final FakeFunctionView<Integer> ageView = new FakeFunctionView<>("age", 1, Integer.class);
	...
	void unboundInVariableTest(List<? extends Literal> clause) {
		...
		if (!clauses.isEmpty()) {
			assertThat(clauses.getFirst().positiveVariables(), hasItem(p));
		}
	}
	...
	void unboundPrivateVariableTest(List<? extends Literal> clause) {
		...
		var clauses = dnf.getClauses();
		if (!clauses.isEmpty()) {
			assertThat(clauses.getFirst().positiveVariables(), not(hasItem(p)));
		}
	}
	...
	void boundPrivateVariableInputTest(Literal literal) {
		...
		var dnf = assertDoesNotThrow(builder::build);
		assertThat(dnf.getClauses().getFirst().positiveVariables(), hasItem(p));
	}
