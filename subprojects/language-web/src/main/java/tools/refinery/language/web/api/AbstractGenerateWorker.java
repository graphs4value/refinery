...
public abstract class AbstractGenerateWorker<T extends GenerateRequest> extends ScheduledWorker<T> {
	private static final Logger LOG = LoggerFactory.getLogger(AbstractGenerateWorker.class);
	...
	protected void run() throws IOException {
		...
		try (var generator = createModelGenerator(problem)) {
			runModelGenerator(generator);
		}
	}

	protected abstract void runModelGenerator(ModelGenerator generator) throws IOException;

	protected void updateStatusString(String status) {
	...
	protected GenerateSuccessResult saveModel(ModelGenerator generator) throws IOException {
		var request = getRequest();
