...
import tools.refinery.language.web.api.dto.ConcretizeSuccessResult;
...
public class ConcretizeWorker extends ScheduledWorker<SemanticsRequest> {
	@Inject
	...
	protected Duration getTimeout() {
		return timeoutManager.getModelConcretizationTimeout();
	}
	...
	protected void initialize(SemanticsRequest request, ResponseSink responseSink) {
		...
		modelSemanticsFactory.concretize(true);
		outputSerializer.setCancellationToken(cancellationToken);
	}
	@Override
	protected void run() throws IOException {
		timeoutManager.markConcretizationAsLoaded();
	...
	private void saveModel(ModelSemantics semantics) throws IOException {
		checkCancelled();
		...
		var source = sourceEnabled ? outputSerializer.saveSource(semantics) : null;
		setResponse(new RefineryResponse.Success(new ConcretizeSuccessResult(issues, json, source)));
	}
}
